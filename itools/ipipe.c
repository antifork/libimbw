...
int             open_dumpfile();
void            pidfile(u_char * pathname);	/* main_thread_pid >
						 * PIDFILE */
...
init_opt(int argc, char **argv)
{
    ...
    while ((c =
	    getopt(argc, argv, "i:S:p:0:1:2:l:G:a:b:s:d:D:e:P:hV")) != EOF)
	switch (c) {
	...
	case 'D':
	    if (o.dumppathname == NULL)
		o.dumppathname = strdup(optarg);
	    break;
...
help()
{
    ...
    printf("  -D pathname     dump connections to \"pathname[0-9]*\"\n");
    printf("  -P pathname     log [main] thread pid\n");
...
open_dumpfile()
{
    ...
    len = strlen(o.dumppathname) + 8;

    if ((pathname = malloc(len)) == NULL)
	fatal("open_dumpfile(): malloc()");

    IMBW_DEBUG("XXXXX %s\n", o.dumppathname);

    for (i = 0; i < NDUMPNAX; ++i) {
	snprintf(pathname, len, "%s%d", o.dumppathname, i);
	pathname[len - 1] = '\0';
    ...
    i = open(pathname, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXU ^ S_IXUSR);
...
void
pidfile(u_char * pathname)
{
    FILE           *f;


    if ((f = fopen(pathname, "w")) == NULL)
	fatal("unable to open '%s'", pathname);
    fprintf(f, "%u\n", getpid());
    fclose(f);
